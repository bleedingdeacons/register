...
public class SnapshotService
{
    ...
    public SnapshotService(UnityDbContext db)
    {
        _db = db;
    }
    ...
    public async Task<SnapshotResult> CaptureAsync(CancellationToken ct = default)
    {
        ...
        // ── Members ──────────────────────────────────────────────────
        var members = await _db.Members.AsNoTracking().ToListAsync(ct);
        ...
        // ── Meetings ─────────────────────────────────────────────────
        var meetings = await _db.Meetings.AsNoTracking().ToListAsync(ct);
        foreach (var m in meetings)
            snapshots.Add(CreateSnapshot("Meeting", m.Id, m, now));

        // ── Contacts ─────────────────────────────────────────────────
        var contacts = await _db.Contacts.AsNoTracking().ToListAsync(ct);
        foreach (var c in contacts)
            snapshots.Add(CreateSnapshot("Contact", c.Id, c, now));

        // ── IntergroupMeetings ───────────────────────────────────────
        var igMeetings = await _db.IntergroupMeetings.AsNoTracking().ToListAsync(ct);
        ...
        await _db.EntitySnapshots.AddRangeAsync(snapshots, ct);
    ...
    {
        return await _db.EntitySnapshots.AnyAsync(ct);
    }
    ...
    {
        return await _db.EntitySnapshots
